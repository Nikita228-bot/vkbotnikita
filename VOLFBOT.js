...
const fs = require("fs"); /////////////////////////////////////////////////////////////////////
const admins = [251931525]; ////// id админа
const vip = [251931525]; /////////// id админа
...
vk.setOptions({
    token: 'd01b1525b8f95b80d266376a2ac31317fe6298f5f7c8620d63badd87be579ee4a953327b03dccd65670b2', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 184689996 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    if (message.is("message") && message.isOutbox)
		...
		message.send(`👴 *id${message.user}, Вы успешно зарегистрированы !\nВас приветсвует Бот Никита!\n✉ Устройтесь на работу, чтобы получить первые деньги ! " Работы "\n👪 Приглашай друзей в бота и получай крутые подарки ! Подробнее " Ринфо "\n👀 ➾ Если возникли вопросы или нашли баг - пишем vk.com/7great7 \n👱 Желаем хорошего настроения :)`)
...
купить: https://vk.com/id515126478
 			`)
 	});
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата закончились деньги ! Попробуй позже!`);
...
vk.updates.hear(/^(?:buyvip)/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(user.donate < 59) return message.send(`💎 У вас не хватает рубинов ! Пополнить счет можно тут: https://qiwi.me/donateunedshop`);
	user.donate -= 59;
	user.level = 1;
	let rez = [true, false].random(); 
	if(rez == false){
...
vk.updates.hear(/^(?:buyglv)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.donate < 60054) return message.send(`💎 Временно не доступно!`);
	user.donate -= 600;
	user.level = 4;
	let rez = [true, false].random();
	if(rez == false){
